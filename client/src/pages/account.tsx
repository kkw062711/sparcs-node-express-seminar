...
const AccountPage = () => {
  const [ SUsername, Setsusername] = React.useState<string>("");
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { credential: {id : SUsername,pw : SPassword} });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { credential: {id : SUsername, pw : SPassword}, amount: amount });
      setNTransaction(0);
      if (!data.success) {
  ...
  return (
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        Enter UserName: <input type={"text"} value={SUsername} onChange={e => Setsusername(e.target.value)}/>
